...
def dowa(matrix,n):
    ...
    i=0
    #opat z definicie caowdt prechadzame podmnoziny vzhladom na indexy
    #a pocitame na kazdej mediany
    for x in coords_all:
        mediany = np.array([np.median(distances[i][:j+1]) for j in range(n)])
        new[x[0]][x[1]] = np.sum(mediany * weights)
        i+=1
    newnew = np.uint8(255 * (new - new.min()) / (new.max() - new.min()))
    im = Image.fromarray(newnew)  
    im.save('caowdt_obr_med.png')
